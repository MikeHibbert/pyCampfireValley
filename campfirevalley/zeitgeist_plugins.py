...
def build_gmail_query(user_text: str, zeitgeist_cfg: Optional[Dict[str, Any]]) -> str:
    ...
    if any(k in low for k in JOB_HINT_KEYWORDS):
        parts.append(
            "{job jobs vacancy vacancies hiring recruiter recruiters "
            "recruitment interview interviews opportunity opportunities "
            "position positions role roles application applications "
            "employer employers candidate hire}"
        )
        _extra = [t for t in terms if t not in JOB_HINT_KEYWORDS]
        if _extra:
            parts.extend(_extra[:4])
    elif terms:
