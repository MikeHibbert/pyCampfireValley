...
class ValleyVisualizer:
    ...
    async def _create_valley_node(self) -> ValleyNode:
        ...
        active_campfires = sum(1 for cf in self.valley.campfires if cf._running)
    ...
    async def _create_campfire_nodes(self) -> List[CampfireNode]:
        ...
        for i, campfire in enumerate(self.valley.campfires):
            # Calculate position in circle
            angle = (2 * math.pi * i) / campfire_count if campfire_count > 0 else 0
            x = radius * math.cos(angle)
            y = radius * math.sin(angle)
            
            campfire_id = f"campfire_{campfire.id}"
    ...
    async def _create_camper_nodes(self) -> List[CamperNode]:
        ...
        for campfire in self.valley.campfires:
            campfire_id = f"campfire_{campfire.id}"
    ...
    async def _create_torch_connections(self) -> List[TorchFlow]:
        ...
        campfires = self.valley.campfires
        
        for i, source_campfire in enumerate(campfires):
            for j, target_campfire in enumerate(campfires):
                if i != j:  # Don't connect to self
                    connection_id = f"torch_flow_{source_campfire.id}_{target_campfire.id}"
                    
                    connection = TorchFlow(
                        id=connection_id,
                        source_id=f"campfire_{source_campfire.id}",
                        target_id=f"campfire_{target_campfire.id}",
    ...
    async def _calculate_valley_health(self) -> float:
        ...
        active_count = sum(1 for cf in self.valley.campfires if cf._running)
        return active_count / len(self.valley.campfires)
